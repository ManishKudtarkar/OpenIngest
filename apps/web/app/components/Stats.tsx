...
const METRICS = [
  ...
  { raw: 32, display: "32", label: "Airflow tasks", sub: "8 datasets x 4", c: "#14B8A6" },
];
...
export default function Stats() {
  ...
  return (
    <section className="relative py-20 overflow-hidden" ref={ref}>
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(20,184,166,0.08),transparent_38%)]" />

      <div className="relative max-w-[1380px] mx-auto px-5 sm:px-6 lg:px-12">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 tag text-cyan-300 border-cyan-300/25 bg-cyan-300/8 mb-4">
            Real metrics / Real run
          </div>
          <h2 className="f-head text-[32px] font-bold text-white">
            By the numbers.
          </h2>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {METRICS.map((m, i) => (
            <div key={m.label}
              className="panel rounded-2xl p-5 text-center relative overflow-hidden group hover:border-white/12 transition-all hover:-translate-y-0.5">
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity"
                style={{ background: `linear-gradient(180deg, ${m.c}10 0%, transparent 72%)` }} />
